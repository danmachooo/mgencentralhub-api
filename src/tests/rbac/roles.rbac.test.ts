...
describe("Roles - admin", async () => {
	...
	it("POST /roles -> 201", async () => {
		expect((await request(app).post("/roles").send({ name: "manager", description: "Manager role" })).status).toBe(
			201
		)
	})
	...
	it("POST /roles/bulk -> 200", async () => {
		expect(
			(
				await request(app)
					.post("/roles/bulk")
					.send([{ name: "viewer", description: "View only" }])
			).status
		).toBe(200)
	})
	...
	it("POST /roles/bulk -> 200", async () => {
		expect(
			(
				await request(app)
					.post("/roles/bulk")
					.send([{ name: "employee-viewer", description: "View only" }])
			).status
		).toBe(200)
	})
