...
import {
	...
	invalidatePersonalSystemCollectionCaches,
	withFavoriteOwnSystemByIdCache,
	withFavoriteOwnSystemListCache,
...
export async function toggleFavoritePersonalSystem(creator: CreatorIdentifier, system: PersonalSystemIdentifier) {
	return personalSystemErrors.exec(async () => {
		const updated = await flipFavoritePersonalSystem(creator.id, system.id)
		await invalidatePersonalFavoriteCache(creator.id, system.id)
		return updated
	})
}
...
export async function restoreOwnSystem(system: PersonalSystemIdentifier, creator: CreatorIdentifier) {
	return personalSystemErrors.exec(async () => {
		...
		await invalidatePersonalSystemCache(creator.id, _system.id)
		return {
...
export async function softDeleteOwnSystem(system: PersonalSystemIdentifier, creator: CreatorIdentifier) {
	return personalSystemErrors.exec(async () => {
		const deleted = await softDeletePersonalSystem(creator.id, system.id)
		await invalidatePersonalSystemCache(creator.id, system.id)
		return deleted
	})
}
...
export async function getOwnSystems(query: PersonalSystemQueryInput, creator: CreatorIdentifier) {
	...
	return withOwnSystemListCache(creator.id, query, () =>
		personalSystemErrors.exec(async () => {
...
export async function getDeletedOwnSystems(query: PersonalSystemQueryInput, creator: CreatorIdentifier) {
	...
	return withOwnDeletedSystemListCache(creator.id, query, () =>
		personalSystemErrors.exec(async () => {
...
export async function getOwnSystemById(system: PersonalSystemIdentifier, creator: CreatorIdentifier) {
	return withOwnSystemByIdCache(creator.id, system.id, () =>
		personalSystemErrors.exec(async () => {
...
export async function getFavoriteOwnSystemById(creator: CreatorIdentifier, system: PersonalSystemIdentifier) {
	return withFavoriteOwnSystemByIdCache(creator.id, system.id, () =>
		personalSystemErrors.exec(async () => {
...
export async function getFavoriteOwnSystems(creator: CreatorIdentifier, query: PersonalSystemQueryInput) {
	...
	return withFavoriteOwnSystemListCache(creator.id, query, () =>
		personalSystemErrors.exec(async () => {
